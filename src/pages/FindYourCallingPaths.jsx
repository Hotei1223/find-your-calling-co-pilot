...
const [email] = useState("ai.studio.projects@gmail.com"); 
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      setLoading(true);
      try {
        const response = await axios.get(` http://localhost:3010/api/fyc/user/${email}`);
        if (response.data) {
  ...
  const handleback = () => {
    ...
    // For example, you might want to navigate to the previous page
    window.history.back();
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(" http://localhost:3010/api/fyc", {
  ...
  const handleProceedStartOver = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(` http://localhost:3010/api/fyc/user/${email}`);
      setFormData({});
      setResponse(null);
      setErrors({});
    } catch (error) {
  ...
  const renderQuestions = (questions, title) => (
    <div className="px-4 py-[25px] w-full bg-[#16171b]/70 rounded-[20px] border border-[#e1e5ea]/40 flex-col justify-start items-start gap-3 inline-flex">
      <div className="self-stretch text-white text-lg font-bold"><h2>{title}</h2></div>
      {questions.map((question) => (
        <div key={question.id} className="w-full flex flex-col gap-2">
          ...
          <h3 style={{ fontSize: '20px' }} className="mt-1 whitespace-pre-wrap font-bold">{question.question}</h3>
    ...
    {question.desc.map((line, index) => (
        <React.Fragment key={index}>
          ...
          />
          {errors[question.id] && <span className="text-red-500 text-sm">This field is required.</span>}
        </div>
      ))}
    </div>
  );
    ...
    <div className="w-full flex justify-center items-start">
      ...
      {!response && !loading && (
        ...
        <div className="flex justify-center items-center w-full h-56 relative rounded-2xl">
          <img src={bannerImg} className="w-full h-full object-cover rounded-2xl" alt="FYC Copilot Banner" />
        </div>
       
        <div className="flex flex-col gap-4">
                    <h4 className="text-white text-lg font-regular tracking-wide leading-[25px]">
                    Answer the following questions to identify your Entrepreneurial Path.
                    </h4>
                </div>
                </>
...
{response && !loading && (
       ...
       <div className="flex flex-col gap-4">
                    ...
                    <button onClick={handleback} className="h-full w-8 border-white border rounded-full text-white p-[6.5px] text-center cursor-pointer hover:bg-white hover:text-black">
                ...
                <>
                  <div className="flex flex-col gap-4">
                    ...
                    Give us a moment, we're putting your <b>Entrepreneurial Path</b> together.....
                    </h3>
                    </div>
                    </>
}
        ...
        {response && !loading && (
          <>
            ...
            <div className="w-full flex justify-center mt-4 space-x-2">
              <button onClick={handleDownloadPDF} className="btn-primary h-10 px-6 rounded-2xl text-white text-sm font-medium">
                Download PDF
              </button>
              <button onClick={handleStartOver} className="btn-primary h-10 px-6 rounded-2xl text-white text-sm font-medium">
                Start Over
              </button>
            </div>
          </>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-[#16171b] p-6 rounded-lg">
              ...
              <p className="text-white mb-6">If you proceed, all your previous answers will be erased. Are you sure you want to start over?</p>
